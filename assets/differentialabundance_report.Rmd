...
# Get blocking variables that exist in observations
blocking_variables <- unique(contrasts$blocking[contrasts$blocking != '' & contrasts$blocking %in% colnames(observations)])

# Get the contrast variables from the design formula
contrast_variables <- unique(contrasts$variable[contrasts$variable %in% colnames(observations)])

# These are the variables we must always include (from the experimental design)
design_variables <- unique(c(contrast_variables, blocking_variables))
...
# variables and blocking variables are first in the list

informative_variables <- unique(c(design_variables, chooseGroupingVariables(observations)))
...
# Ensure design variables are treated as factors (important for numeric-looking columns like batch)
for (dv in design_variables) {
    if (dv %in% colnames(observations)) {
        observations[[dv]] <- as.factor(observations[[dv]])
    }
}
...
# Show the variable with the tightest PC associations first, but always keep design variables
anova_ordered_variables <- rownames(pca_vs_meta)[order(pca_vs_meta[,1])]
informative_variables <- unique(c(design_variables, anova_ordered_variables))
...
for (assay_type in rev(names(assay_data))){
    for (iv in informative_variables){
        ...
        # Calculate number of legend columns based on number of categories
        n_categories <- length(unique(observations[[iv]]))
        legend_ncol <- ceiling(n_categories / 10)  # Max ~10 items per column
        ...
        # Modify the legend to use multiple columns if there are many categories
        if (n_categories > 10) {
            p <- p + ggplot2::guides(color = ggplot2::guide_legend(ncol = legend_ncol))
        }
...
# Add batch-corrected exploratory plots if blocking variables are present
if (length(blocking_variables) > 0) {
    cat("\n### Batch-corrected exploratory analysis\n")
    cat("\nThe following plots show the variance-stabilised data after removing the effect of blocking factor(s) using `limma::removeBatchEffect()`. This is useful for visualisation purposes to assess whether samples cluster by biological variables of interest after accounting for technical variation.\n\n")
    cat("**Note:** This batch correction is applied only for visualisation. The actual differential expression analysis accounts for batch effects through the model design formula. Plots colored by the blocking variable are included as a sanity check - samples should no longer cluster by this variable after correction.\n")
}
```

```{r, echo=FALSE, results='asis', warning=FALSE}
# Generate batch-corrected data and plots for each blocking variable
if (length(blocking_variables) > 0) {
    
    # Get the variance-stabilised assay
    vst_assay_name <- params$exploratory_final_assay
    vst_matrix <- assay_data[[vst_assay_name]]
    
    for (block_var in blocking_variables) {
        
        cat(paste0("\n#### Corrected for: ", block_var, "\n"))
        
        # Create batch factor
        batch_factor <- as.factor(observations[[block_var]])
        
        # Apply limma::removeBatchEffect
        corrected_matrix <- limma::removeBatchEffect(vst_matrix, batch = batch_factor)
        
        # Run PCA on corrected data
        pca_corrected <- compilePCAData(corrected_matrix)
        
        # Generate PCA plots for each informative variable (including the blocking variable as a sanity check)
        plot_variables <- informative_variables
        
        cat("\n##### PCA plots {.tabset}\n")
        
        for (iv in plot_variables) {
            
            cat(paste0("\n###### ", iv, "\n"))
            
            plotdata <- pca_corrected$coords
            plotdata$colorby <- factor(
                observations[[iv]],
                levels = unique(observations[[iv]])
            )
            ...
            plotdata$name <- rownames(plotdata)
            percentVar <- pca_corrected$percentVar
            labels <- paste0(colnames(plotdata), " (", sprintf("%.1f", percentVar), "%)")
            ...
            for (d in names(plot_types)) {
                plot_args <- list(
                    x = pca_corrected$coords[, 1],
                    y = pca_corrected$coords[, 2],
                    xlab = labels[1],
                    ylab = labels[2],
                    colorby = plotdata$colorby,
                    plot_type = plot_types[[d]],
                ...
                if (d == "3") {
                    plot_args$z <- pca_corrected$coords[, 3]
                    plot_args$zlab <- labels[3]
                }
        ...
        cat("\n##### Principal components/ metadata associations (batch-corrected)\n")
        cat(paste0("\nFor the batch-corrected variance stabilised matrix (after removing ", block_var, " effect), an ANOVA test was used to determine associations between continuous principal components and categorical covariates.\n\nThe resulting p values are illustrated below.\n\n"))
        
        # Calculate PCA vs metadata associations for corrected data
        pca_vs_meta_corrected <- anova_pca_metadata(
            pca_corrected$coords, 
            observations[, plot_variables, drop = FALSE], 
            pca_corrected$percentVar
        )
        
        if (nrow(pca_vs_meta_corrected) == 1) {
            plot_pca_meta_corrected <- rbind(pca_vs_meta_corrected, pca_vs_meta_corrected)
        } else {
            plot_pca_meta_corrected <- pca_vs_meta_corrected
        }
    }
}
```

```{r, echo=FALSE, results='asis', eval=(length(blocking_variables) > 0)}
# This chunk renders the batch-corrected PCA-metadata heatmap
# Use the first blocking variable (typically there's only one)
block_var <- blocking_variables[1]

batch_factor <- as.factor(observations[[block_var]])
vst_matrix <- assay_data[[params$exploratory_final_assay]]
corrected_matrix <- limma::removeBatchEffect(vst_matrix, batch = batch_factor)
pca_corrected <- compilePCAData(corrected_matrix)
plot_variables <- informative_variables
...
# Render the heatmap directly (not inside any conditional)
d3heatmap::d3heatmap(
    -log10(plot_pca_meta_corrected),
    Rowv = FALSE,
    dendrogram = 'none',
    cellnote = plot_pca_meta_corrected,
    cexCol = 0.8,
    cexRow = 0.8,
    height = (100 + (15 * nrow(plot_pca_meta_corrected))),
    colors = colorRampPalette(
...
# Continue with the rest of the batch-corrected section
if (length(blocking_variables) > 0) {
    vst_assay_name <- params$exploratory_final_assay
    vst_matrix <- assay_data[[vst_assay_name]]
    ...
    for (block_var in blocking_variables) {
        ...
        for (variable in rownames(pca_vs_meta_corrected)) {
            sig_comps <- pca_vs_meta_corrected[variable, ] < 0.1
            if (any(sig_comps)) {
                min_sig_comp <- min(which(sig_comps))
                min_sig_comp_p <- sprintf("%.2f", pca_vs_meta_corrected[variable, min_sig_comp])
                cat(paste0('After ', block_var, ' correction, the variable \'', variable, '\' shows an association with ', colnames(pca_vs_meta_corrected)[min_sig_comp], ' (p = ', min_sig_comp_p, '). '))
            }
        }
        cat("\n")
        ...
        # Generate dendrograms for each informative variable
        for (iv in plot_variables) {
            ...
            variable_genes <- selectVariableGenes(
                ...
                ntop = ifelse(params$exploratory_n_features == -1, nrow(corrected_matrix), params$exploratory_n_features)
            )
            ...
            p <- clusteringDendrogram(
                2^corrected_matrix[variable_genes, ],
                observations[, iv, drop = FALSE],
                colorby = iv,
                cor_method = params$exploratory_cor_method,
                plot_title = paste0(
                    params$observations_type, " clustering dendrogram (", block_var, "-corrected)\n",
                    ifelse(params$exploratory_n_features == -1, nrow(corrected_matrix), paste0(params$exploratory_n_features, " most variable")), " ",
                    params$features_type, "s\n(",
                    params$exploratory_clustering_method, " clustering, ", params$exploratory_cor_method, " correlation)"
                ),
                cluster_method = params$exploratory_clustering_method,
                palette = dendroColorScale,
